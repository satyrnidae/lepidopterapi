...
@Transformation(rotation = true, offset = true, scale = true)
public final class RotationOffsetScale implements NestingConfigData<RotationOffsetScale> {
    ...
    public RotationOffsetScale(final float rotX,
                               ...
                               final int offsetZ,
                               final float scale) {
        this(new Rotation(rotX, rotY, rotZ), new Offset(offsetX, offsetY, offsetZ), scale);
    }

    private RotationOffsetScale(final Rotation rotation,
                                final Offset offset,
                                final float scale) {
     ...
     */
    @Override
    public void copyFrom(RotationOffsetScale other) {
    ...
    public enum Codec implements ConfigCodec<RotationOffsetScale> {
        INSTANCE;
         ...
         */
        @Override
        public void encode(RotationOffsetScale value, FriendlyByteBuf buf) {
         ...
         */
        @Override
        public RotationOffsetScale decode(FriendlyByteBuf buf) {
            final var rotation = Rotation.Codec.INSTANCE.decode(buf);
            final var offset = Offset.Codec.INSTANCE.decode(buf);
            return new RotationOffsetScale(rotation, offset, buf.readFloat());
        }
    }
}
