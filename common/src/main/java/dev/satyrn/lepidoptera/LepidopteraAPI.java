...
public class LepidopteraAPI implements LepidopteraMod {
    ...
    public static final ResourceLocation ALCHEMICAL_ALEMBIC_RECIPES_CONDITION =
            ResourceLocation.fromNamespaceAndPath(MOD_ID, "alchemical_alembic_recipes");
     ...
     */
    public static SyncedConfig<LepidopteraConfig> SYNCED_CONFIG =
            new SyncedConfig<>(new LepidopteraConfig(), new ConfigOverlay<>());
    ...
    public void preInit() {
        ...
        AutoConfig.register(LepidopteraConfig.class,
                (def, cls) -> new CommentedYamlConfigSerializer<>(def, cls, CommentedYamlConfigSerializer.DEFAULT_LINE_LENGTH));
    ...
    public void init() {
        ...
        SYNCED_CONFIG = builder.clientOverride(() -> true, LepidopteraConfig.Codec.INSTANCE, local)
                .onApply(cfg -> {
                    EquipmentRegistry.updateEquipment(EquipmentSlot.HEAD,
                            LepidopteraItems.ALCHEMICAL_ALEMBIC.get(), cfg.alchemicalAlembicCanShiftClick);
                    EquipmentRegistry.updateEquipment(EquipmentSlot.HEAD,
                            LepidopteraItems.DEPLETED_ALEMBIC.get(), cfg.alchemicalAlembicCanShiftClick);
                })
                .onClear(() -> {
                    LepidopteraConfig localCfg = AutoConfig.getConfigHolder(LepidopteraConfig.class).getConfig();
                    EquipmentRegistry.updateEquipment(EquipmentSlot.HEAD,
                            LepidopteraItems.ALCHEMICAL_ALEMBIC.get(), localCfg.alchemicalAlembicCanShiftClick);
                    EquipmentRegistry.updateEquipment(EquipmentSlot.HEAD,
                            LepidopteraItems.DEPLETED_ALEMBIC.get(), localCfg.alchemicalAlembicCanShiftClick);
                });
    ...
    public void serverStarted(MinecraftServer server) {
        ...
        if (CONFIG_SYNC != null) CONFIG_SYNC.startWatching(server);
    }

    @Override
    public void serverStopped() {
        if (CONFIG_SYNC != null) CONFIG_SYNC.stopWatching();
        currentServer = null;
    }
