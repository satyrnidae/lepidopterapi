...
import dev.satyrn.lepidoptera.fabric.network.play.FabricPacketChannelsImpl;
...
public final class MainEntrypoint implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        CommonLifecycleEvents.TAGS_LOADED.register((registryAccess, client) -> {
            if (!client) LepidopteraAPI.INSTANCE.onTagsLoaded(registryAccess);
        });
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            FabricPacketChannelsImpl.FabricServerPlayContext ctx =
                    new FabricPacketChannelsImpl.FabricServerPlayContext(handler.getPlayer(), server);
